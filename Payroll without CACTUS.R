...
# create a list of all employees for each fiscal year 
payroll_namesbyyear <- payroll %>% distinct(Fiscal_Year, Employee_ID, Name)
...
for(year in min_year:max_year){ 
  ...
  course_cactus_year <- course_cactus %>% filter(SCHOOL_YEAR == year) 
  payroll_names_year <- payroll_namesbyyear %>% filter(Fiscal_Year == year)
...
# teachers_only %>% distinct(OBJECT,Description) %>% arrange(OBJECT) %>% View()

# idenfity exceptions to default assignments based on function & description: 
# teachers_only %>% distinct(FUNCTION, Description) %>% arrange(FUNCTION) %>% View()

## Which descriptions are new each year? What object codes are new? Functions?

# check new descriptions in latest year
# teachers_only %>% distinct(Description, Fiscal_Year, type) %>% group_by(Description) %>% mutate(count = length(unique(Fiscal_Year))) %>% ungroup() %>%
...
# teachers_only %>% distinct(FUNCTION, OBJECT, Description, Fiscal_Year, type) %>% group_by(FUNCTION, OBJECT, Description) %>% mutate(count = length(unique(Fiscal_Year))) %>% ungroup() %>%
#   filter(Fiscal_Year == max_year & count == 1) %>% select(-count) %>% View()
